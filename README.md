...
docker compose up -d            # 首次：容器往 data/config.yaml 写一份模板，退出码 2 一次，
                                # 然后重启并以该模板正常启动（Web UI 立即可用）
$EDITOR data/config.yaml        # 填真实 xteink 手机号/密码、（可选）AI 接口，加 RSS 源和任务
chmod 600 data/config.yaml      # 里面是明文密钥，见下方「安全须知」
# 保存后 ~5s 内热加载，无需重启容器（也可在 Web 的 Settings 页改）
...
`morning-brief` 这个示例 cron 任务也已注册。它**不会**进入重启循环。
